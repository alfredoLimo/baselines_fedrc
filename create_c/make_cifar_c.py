...
def get_noniid_class_and_labels2(original_images, original_labels, N):
    # Calculate the number of samples per client
    M = len(original_labels) // N

    # Initialize lists to hold images and labels for each client
    clients_images = [[] for _ in range(N)]
    clients_labels = [[] for _ in range(N)]

    # Iterate through each client and assign M samples to each
    for i in range(N):
        start_index = i * M
        end_index = start_index + M
        clients_images[i] = original_images[start_index:end_index]
        clients_labels[i] = original_labels[start_index:end_index]
...
clients_images, clients_labels = get_noniid_class_and_labels2(original_images_tr, original_labels_tr, client_number)
